...
- MUST accept environment variables for configuration, unless user specifically opts out.
- MUST validate input paths exist and are readable.
- Sensitive data must use environment variables.
- Environment variable names MUST use a project prefix [A-Z][A-Z0-9_]*_ (e.g., SPLURGE_DSV_).
- Provide --output-format {table, json, or ndjson} (default: table).
