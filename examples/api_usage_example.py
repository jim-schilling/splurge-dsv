...
Line 4: Final line"""
    ...
    try:
        ...
        content_with_header = TextFileHelper.read(temp_path, skip_header_rows=1)
...
def demonstrate_dsv_parsing() -> None:
    ...
    try:
        ...
        parsed_no_header = DsvHelper.parse_file(temp_path, delimiter=",", skip_header_rows=1)
        print(f"✓ Parsed without header: {len(parsed_no_header)} rows")

        # Parsing with custom options
        parsed_custom = DsvHelper.parse_file(temp_path, delimiter=",", strip=True)
...
# End of data"""
    ...
    try:
        ...
        parsed_data = DsvHelper.parse_file(temp_path, delimiter=",", skip_header_rows=1, strip=True)
